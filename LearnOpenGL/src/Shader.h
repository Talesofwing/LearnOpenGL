...
public:
    ...
    Shader(const char* vertexPath, const char* fragmentPath, const char *geometryPath = nullptr) {
        ...
        try {
            // open files
            std::string vertexFullpath = "src/shaders/" + std::string(vertexPath);
            std::string fragmentFullpath = "src/shaders/" + std::string(fragmentPath);
            ...
            if (geometryPath != nullptr) {
                std::string geometryFullpath = "src/shaders/" + std::string(geometryPath);
        ...
        if (geometryPath != nullptr) {
            ...
            checkCompileErrors(geometry, std::string("GEOMTRY: ") + geometryPath);
        }
        // shader Program
        ID = glCreateProgram();
    ...
    void setVec2(const std::string& name, float x, float y) const {
        glUniform3fv(glGetUniformLocation(ID, name.c_str()), 1, glm::value_ptr(glm::vec2(x, y)));
    }
